...
import com.hbm.inventory.menus.MachinePrecisionAssemblyMachineMenu;
import com.hbm.inventory.recipes.PrecisionAssemblyMachineRecipes;
...
public class MachinePrecisionAssemblyMachineScreen extends InfoScreen<MachinePrecisionAssemblyMachineMenu> {

    private static final ResourceLocation TEXTURE = NuclearTechMod.withDefaultNamespace("textures/gui/processing/precass.png");

    private final MachinePrecisionAssemblyMachineBlockEntity be;

    public MachinePrecisionAssemblyMachineScreen(MachinePrecisionAssemblyMachineMenu menu, Inventory inventory, Component title) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if(this.checkClick(mouseX, mouseY, 7, 125, 18, 18)) {
            RecipeSelectorScreen.openSelector(PrecisionAssemblyMachineRecipes.INSTANCE, be, be.assemblerModule.recipe, 0, BlueprintsItem.grabPool(be.slots.get(1)), this);
        }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        if(be.assemblerModule.progress > 0) {
            int progress = (int) Math.ceil(70 * be.assemblerModule.progress);
            guiGraphics.blit(TEXTURE, this.leftPos + 62, this.topPos + 126, 176, 61, progress, 16);
        }

        GenericRecipe recipe = be.assemblerModule.getRecipe();

        if(be.didProcess) guiGraphics.blit(TEXTURE, this.leftPos + 51, this.topPos + 121, 195, 0, 3, 6);
        else if(recipe != null) guiGraphics.blit(TEXTURE, this.leftPos + 51, this.topPos + 121, 192, 0, 3, 6);

        if(be.didProcess) guiGraphics.blit(TEXTURE, this.leftPos + 56, this.topPos + 121, 195, 0, 3, 6);
        else if(recipe != null && be.power >= recipe.power) guiGraphics.blit(TEXTURE, this.leftPos + 56, this.topPos + 121, 192, 0, 3, 6);
