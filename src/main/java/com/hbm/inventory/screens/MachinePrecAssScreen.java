...
public class MachinePrecAssScreen extends InfoScreen<MachinePrecAssMenu> {

    private static final ResourceLocation TEXTURE = NuclearTechMod.withDefaultNamespace("textures/gui/processing/precass.png");

    private final MachinePrecAssBlockEntity be;

    public MachinePrecAssScreen(MachinePrecAssMenu menu, Inventory inventory, Component title) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if(this.checkClick(mouseX, mouseY, 7, 125, 18, 18)) {
            RecipeSelectorScreen.openSelector(PrecAssRecipes.INSTANCE, be, be.assemblerModule.recipe, 0, BlueprintsItem.grabPool(be.slots.get(1)), this);
        }
