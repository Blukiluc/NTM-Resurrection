...
public class MachineSolderingStationMenu extends MenuBase<MachineSolderingStationBlockEntity> {
    ...
    public MachineSolderingStationMenu(int id, Inventory inventory, MachineSolderingStationBlockEntity be) {
        super(NtmMenuTypes.SOLDERING_STATION.get(), be, id);
        ...
        if(slot.hasItem()) {
            ...
            if(index <= 10) {
                ...
                if(stack.getItem() instanceof IBatteryItem) {
                    ...
                    for(com.hbm.inventory.RecipesCommon.AStack t : SolderingStationRecipes.toppings) {
                        if(t.matchesRecipe(stack, false) && this.moveItemStackTo(stack, 0, 3, false)) {
                            break;
                        }
                    }
                    if(!stack.isEmpty()) {
                        for(com.hbm.inventory.RecipesCommon.AStack t : SolderingStationRecipes.pcb) {
                    ...
                    if(!stack.isEmpty()) {
                        for(com.hbm.inventory.RecipesCommon.AStack t : SolderingStationRecipes.solder) {
