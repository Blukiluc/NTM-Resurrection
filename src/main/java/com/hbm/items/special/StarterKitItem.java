...
public class StarterKitItem extends EnumMultiItem {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> components, TooltipFlag flag) {
        KitType type = EnumUtil.grabEnumSafely(KitType.class, MetaHelper.getMeta(stack));
        if(type.emptyInv) {
            for(String s : I18nUtil.resolveKeyArray("item." + NuclearTechMod.MODID + ".obj_starter_kit.empty_inventory")) components.add(Component.translatable(s).withStyle(ChatFormatting.GRAY));
        }
    }
}
